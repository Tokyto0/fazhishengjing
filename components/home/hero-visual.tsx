...
export function HeroVisual() {
  return (
    <div className="relative mx-auto aspect-square w-full max-w-[520px]" aria-hidden="true">
      <motion.div
        className="absolute inset-[11%] rounded-full border border-emerald-300/40"
        animate={{ rotate: 360 }}
        transition={{ duration: 34, repeat: Infinity, ease: "linear" }}
      >
        <span className="absolute left-1/2 top-0 h-2.5 w-2.5 -translate-x-1/2 -translate-y-1/2 rounded-full bg-mint shadow-[0_0_18px_#76c893]" />
      </motion.div>
      <motion.div
        className="absolute inset-[23%] rounded-full border border-dashed border-emerald-300/30"
        animate={{ rotate: -360 }}
        transition={{ duration: 24, repeat: Infinity, ease: "linear" }}
      />
      <div className="absolute inset-[31%] grid place-items-center rounded-[38%] border border-white/60 bg-white/80 shadow-[0_30px_100px_rgba(15,75,46,.22)] backdrop-blur-xl">
      ...
      <svg className="absolute inset-0 h-full w-full opacity-50" viewBox="0 0 100 100">
        <path d="M21 27 L40 40 M79 24 L61 40 M79 76 L62 61 M20 76 L39 61" fill="none" stroke="#17875f" strokeWidth=".35" strokeDasharray="2 2" />
      </svg>
      {nodes.map((node) => {
