  ...
  return (
    <PageTransition>
      <article className="bg-white pt-[76px]">
        ...
        <div className="mx-auto grid max-w-5xl gap-10 px-5 py-14 sm:px-8 lg:grid-cols-[1fr_240px] lg:py-20"><div className="prose-fazhi"><ReactMarkdown remarkPlugins={[remarkGfm]}>{article.body}</ReactMarkdown></div><aside className="space-y-4 lg:sticky lg:top-28 lg:self-start"><div className="rounded-2xl border border-blue-100 bg-blue-50 p-5"><Info className="h-5 w-5 text-signal" /><p className="mt-3 text-sm font-bold text-ocean">使用说明</p><p className="mt-2 text-xs leading-6 text-slate-600">内容用于公益普法与学习交流，不构成针对个案的法律意见。法条与政策请以现行官方文本为准。</p></div>{article.resourceUrl && <a href={article.resourceUrl} download className="button-secondary w-full"><Download className="h-4 w-4" />下载配套资料</a>}<div className="flex flex-wrap gap-2 pt-2">{article.tags.map((tag) => <span key={tag} className="rounded-full bg-slate-100 px-3 py-1 text-xs text-slate-500">#{tag}</span>)}</div></aside></div>
      </article>
    </PageTransition>
  );
}
