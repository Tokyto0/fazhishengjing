...
const entries = [
  { href: "/research", title: "五省区调研纪实", description: "在地图与时间线中回到实践现场", icon: MapPinned, number: "01", color: "bg-emerald-50 text-emerald-700" },
...
export default function HomePage() {
  ...
  return (
    <PageTransition>
      <section className="relative min-h-[760px] overflow-hidden bg-[linear-gradient(135deg,#f8fdf9_0%,#edf8f0_46%,#f4fbf5_100%)] pt-[76px]">
        ...
        <div className="absolute -left-32 top-24 h-[420px] w-[420px] rounded-full bg-emerald-200/30 blur-3xl" />
        ...
        <div className="container-page relative grid min-h-[680px] items-center gap-6 py-16 lg:grid-cols-[1.05fr_.95fr] lg:py-20">
          <Reveal>
            <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-emerald-200 bg-white/80 px-4 py-2 text-xs font-semibold tracking-wide text-ocean shadow-sm backdrop-blur">
            ...
            <h1 className="max-w-3xl text-balance font-display text-5xl font-bold leading-[1.08] tracking-tight text-ink sm:text-6xl lg:text-[76px]">
              ...
              <span className="relative text-ocean">
                被看见，被保护
                <svg className="absolute -bottom-2 left-0 w-full" viewBox="0 0 500 16" fill="none" aria-hidden="true"><path d="M3 12C132 3 336 3 497 8" stroke="#76c893" strokeWidth="5" strokeLinecap="round" opacity=".8" /></svg>
              </span>
            </h1>
            ...
            {entries.map((item, index) => {
              ...
              return (
                <Reveal key={item.href} delay={index * 0.05} className={wide ? "lg:col-span-3" : "lg:col-span-2"}>
                  <Link href={item.href} className="group flex h-full min-h-[230px] flex-col justify-between rounded-2xl border border-slate-200 bg-white p-7 transition duration-300 hover:-translate-y-1 hover:border-emerald-200 hover:shadow-soft">
                    <div className="flex items-start justify-between">
                      <span className={`grid h-12 w-12 place-items-center rounded-xl ${item.color}`}><Icon className="h-5 w-5" /></span>
                      <span className="font-display text-4xl text-slate-100 transition group-hover:text-emerald-100">{item.number}</span>
                    </div>
                    <div>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {researchStops.map((stop, index) => (
              <Reveal key={stop.id} delay={index * 0.06}>
                <div className="rounded-2xl border border-white/10 bg-white/[0.06] p-6 transition hover:border-emerald-400/40 hover:bg-white/[0.09]">
                  <div className="flex items-center justify-between text-xs"><span className="font-semibold tracking-wider text-emerald-300">{stop.province}</span><span className="text-slate-500">0{index + 1}</span></div>
            ...
            {featuredCases.map((item, index) => (
              <Reveal key={item.slug} delay={index * 0.06}>
                <Link href={`/cases/${item.slug}`} className="group flex min-h-[340px] flex-col rounded-2xl border border-slate-200 bg-white p-7 shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-soft">
                  <div className="flex items-center justify-between"><span className="rounded-full bg-emerald-50 px-3 py-1 text-xs font-semibold text-emerald-700">案例专题</span><span className="font-mono text-xs text-slate-400">{String(item.order).padStart(2, "0")}</span></div>
